...
Replay the sanitized live-mode success sample:

```bash
python -m spectrail extract docs/sample_srs.md \
  --model-mode recorded \
  --recorded-fixture fixtures/recorded/sample_srs_live_deepseek_response.json \
  --output outputs/demo_recorded_live_output
```
