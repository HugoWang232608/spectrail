...
class OpenAICompatibleTransport:
    ...
    def complete(self, request: CompletionRequest) -> CompletionResponse:
        if not request.prompt.strip():
            raise ModelConfigurationError("completion prompt must not be empty")
        ...
        body = OPENAI_COMPATIBLE_ADAPTER.build_provider_request_body(
    ...
    def _load_config(self, *, insecure: bool = False) -> dict[str, Any]:
        ...
        api_key = (
            self.api_key
            or os.environ.get("SPECTRAIL_LLM_API_KEY")
            or dotenv.get("SPECTRAIL_LLM_API_KEY")
        )
        if not api_key:
            raise ModelConfigurationError(
                "SPECTRAIL_LLM_API_KEY is required for live mode"
            )

        model_name = (
            self.model_name
            or os.environ.get("SPECTRAIL_LLM_MODEL")
            or dotenv.get("SPECTRAIL_LLM_MODEL")
        )
        if not model_name:
            raise ModelConfigurationError("SPECTRAIL_LLM_MODEL is required for live mode")

        timeout_raw = (
            os.environ.get("SPECTRAIL_LLM_TIMEOUT_SECONDS")
            or dotenv.get("SPECTRAIL_LLM_TIMEOUT_SECONDS")
        )
        ...
        if timeout_seconds is None and timeout_raw:
            try:
                ...
                raise ModelConfigurationError(
                    "SPECTRAIL_LLM_TIMEOUT_SECONDS must be a number"
                ) from exc
