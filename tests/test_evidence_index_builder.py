...
@pytest.mark.parametrize(
    ("parser_identity", "message"),
    [
        (
            ParserIdentity(
                parser_name="custom_markdown_parser_v9",
                parser_version="9",
                source_format="markdown",
            ),
            "parser_name does not match parsed document parser_name",
        ),
        (
            ParserIdentity(
                parser_name="markdown_parser_v1",
            ...
            "source_format does not match parsed document source_format",
        ),
    ],
)
def test_parsed_document_parser_identity_must_match_top_level_identity(
    tmp_path: Path,
    ...
    document = tmp_path / "sample.md"
    document.write_text("hello", encoding="utf-8")
    parsed = replace(_parsed(document), parser_identity=parser_identity)

    with pytest.raises(ValueError, match=message):
        ensure_evidence_index(document, parsed)
...
def _parsed_table_with_duplicate_and_empty_occurrences() -> tuple[ParsedDocument, EvidenceIndex]:
    text = "HeaderHeader"
    block = DocumentBlock(
        block_id="blk_0001",
        document_id="doc_001",
...
def test_cell_occurrences_match_logical_cell_text_including_duplicate_and_empty():
